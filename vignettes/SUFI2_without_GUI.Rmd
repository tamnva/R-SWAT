---
title: "Run SWAT/SWAT+ without GUI using SUFI-2 approach"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Run SWAT/SWAT+ without GUI using SUFI-2 approach}
...
### 1. Overview
This documents describe how to performe parameter calibration and uncertainty 
analysis for SWAT, SWAT+, or its modified versions (e.g., SWAT-Carbon, etc.)


### 2. Prepare example data
In this example, we are going to simulate streamflow. Required input data are 
all files in the TxtInOut, observed streamflow, and parameter file of SWAT 
(swatParam) an SWAT+ (cali_parms.cal). These data are included in 
this package.
...
Now setup R-SWAT for parameter calibratoin and sensitivity analysis. Here we
follow the same sequence of step as the GUI of R-SWAT.
...
### 4. Read data, generate parameters, and run the model
The below code will be run automatically with R-SWAT GUI, however, when working
with R script, users need to code this part (in this case, the code is shown
below, no changes is required)
