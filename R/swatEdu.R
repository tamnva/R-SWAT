...
plotSwatEdu <- function(globalVariable, varNr){
  ...
  myPlot <- ggplot(data) +                   
    geom_point(aes(x  = date, y = observed, color = "observed"), alpha = 0.65, size = 0.5) + 
    geom_line(aes(x  = date, y = simulated, color = Flag), alpha = 0.9, linewidth = 0.5)+
    labs(x  = "", y = paste("variable", varNr), color = "")+
